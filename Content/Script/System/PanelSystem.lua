...
function PanelSystem:OpenPanel(PanelKey, ...)
    ...
    if not PanelObject then
        ...
        PanelObject = UE.UWidgetBlueprintLibrary.Create(_G.World, PanelClass)
        PanelObject:AddToViewport(PanelData.ZOrder)
...
function PanelSystem:CloseAll()
    ...
    for _, PanelObject in pairs(self._PanelDict) do
        if UE.UKismetSystemLibrary.IsValid(PanelObject) then
            PanelObject:RemoveFromParent()
        end
    end
    self._PanelDict = {}
end
